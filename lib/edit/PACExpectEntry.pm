...
sub get_cfg {
	...
	foreach my $w ( @{ $self -> {list} } ) {
		...
		if ( $$w{cbOnMatch} -> get_active )	{ $hash{on_match} = $$w{rbOnMatchGoto} -> get_active ? $$w{on_match} -> get_chars( 0, -1 ) : -2; }
		else								{ $hash{on_match} = -1; }
		
		if ( $$w{cbOnFail} -> get_active )	{ $hash{on_fail} = $$w{rbOnFailGoto} -> get_active ? $$w{on_fail} -> get_chars( 0, -1 ) : -2; }
		else								{ $hash{on_fail} = -1; }
