...
sub update {
	...
	$$self{gui}{entrySelectWords}			-> set_text( $$cfg{'terminal select words'} // '\.:_\/-A-Za-z0-9' );
				...
				$w{vbox1} -> pack_start( $frameTermUI, 0, 1, 0 );
					...
					$frameTermUI -> add ( $vboxTermUI );
						...
						$hboxTermUI2 -> set_border_width( 5 );
							...
							my $frameSelectWords = Gtk2::Frame -> new( ' Select Word CHARS: ' );
							$hboxTermUI2 -> pack_start( $frameSelectWords, 0, 1, 0 );
								
								$w{entrySelectWords} = Gtk2::Entry -> new;
