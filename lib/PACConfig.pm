...
sub _initGUI {
	...
	# Load XML Glade file
	defined $$self{_GLADE} or $$self{_GLADE} = Gtk3::Builder -> new();
	$$self{_GLADE} -> add_from_file ($GLADE_FILE ) or die "ERROR: Could not load GLADE file '$GLADE_FILE' ($!)";
...
sub _exporter {
	...
	if ( ! $file ) {
		...
		$$self{_WINDOWCONFIG} -> window -> set_cursor( Gtk3::Gdk::Cursor -> new( 'watch' ) );
	...
	#~ defined $$self{_WINDOWCONFIG} -> window and $$self{_WINDOWCONFIG} -> window -> set_cursor( Gtk3::Gdk::Cursor -> new( 'left-ptr' ) );
